...
static void copy_fsm(int fd1, int fd2)
{
	...
	while (fsm1.state != STATE_TERM && fsm2.state != STATE_TERM) {
		...
		// 以阻塞方式监视
		int maxfd = fsm1.source > fsm1.dest ? fsm1.source : fsm1.dest;
		int ret = 0;
		do {
			// 每次循环开始都需要初始化集合，因为select会覆盖
			FD_ZERO(&rset);
			FD_ZERO(&wset);

			// 布置监视任务
			if (fsm1.state == STATE_READ) {
				FD_SET(fsm1.source, &rset);
			}
			if (fsm1.state == STATE_WRITE) {
				FD_SET(fsm1.dest, &wset);
			}
			if (fsm2.state == STATE_READ) {
				FD_SET(fsm2.source, &rset);
			}
			if (fsm2.state == STATE_WRITE) {
				FD_SET(fsm2.dest, &wset);
			}
		...
		// 查看监视结果
		if (FD_ISSET(fsm1.source, &rset) ||
		    FD_ISSET(fsm1.dest, &wset)) {
			fsm_driver(&fsm1);
		}
		if (FD_ISSET(fsm2.source, &rset) ||
		    FD_ISSET(fsm2.dest, &wset)) {
			fsm_driver(&fsm2);
		}
	}
